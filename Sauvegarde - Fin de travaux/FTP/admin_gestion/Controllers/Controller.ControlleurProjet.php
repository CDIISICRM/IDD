...
// controller=Projet&action=AfficherTousProjets&page=1&pagination=5
	...
	public function DetailProjet($id)
		{
		$motCle = false;
		if(isset($_GET['mot_cle']) && $_GET['mot_cle'] != '')
			$motCle = $_GET['mot_cle'];
		...
		if(is_file($img1))
			...
			$connection = Connect::getInstance();
			$req = "SELECT p.nom, p.logo, p.sygle, p.siteInternet FROM partenaires AS p, agit AS a WHERE p.id = a.idPartenaire AND a.idProjet=".$projet;
			...
			if($query->num_rows > 0)
				...
				while($data = $query->fetch_array())
					...
					$siteWeb = '';
					if($data[3] != '')
						$siteWeb = ' : <a href="'.$data[3].'" target="_blank">Site Internet</a>';
					
					$listePartenaire .= '<li>'.$data[0].' - <img alt="'.$data[2].'" src="'.$data[1].'" width="30"/>'.$siteWeb.'</li>';
					}
				}
			// idPartenaire
			// idProjet
			...
			if($motCle)
				$backLink = "window.location.href='index.php?controller=Rechercher&action=RechercherProjet&mot_cle=".$motCle."'";
